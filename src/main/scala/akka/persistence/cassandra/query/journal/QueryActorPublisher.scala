...
import akka.persistence.cassandra.query.journal.QueryActorPublisher.DelierRequest
...
abstract class QueryActorPublisher[MessageType, State](refreshInterval: Option[FiniteDuration], maxBufSize: Int, override val writeJournalPluginId: String)
  ...
  def streaming(buffer: Vector[MessageType], state: State): Receive = {
    ...
    case BufferUpdate(current) ⇒
      ...
      if (isStreamComplete(newBuf)) onCompleteThenStop()
      context.become(streaming(newBuf, newState))
      //self ! DelierRequest
