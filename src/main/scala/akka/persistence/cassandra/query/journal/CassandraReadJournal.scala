...
object CassandraReadJournal {
   ...
   *
   * The value is `"akka.persistence.query.journal.Cassandra"` and corresponds
   * to the absolute path to the read journal configuration entry.
   */
  final val Identifier = "akka.persistence.query.journal.cassandra"
}

/**
 * [[akka.persistence.query.scaladsl.ReadJournal]] implementation for Cassandra.
 *
 ...
 * {{{
 * ReadJournal queries =
 *   PersistenceQuery.get(system).getReadJournalFor(CassandraReadJournal.Identifier());
 * }}}
 *
 * Configuration settings can be defined in the configuration section with the
 * absolute path corresponding to the identifier, which is `"akka.persistence.query.journal.Cassandra"`
 ...
 * </ul>
 *
 * The Cassandra write journal is notifying the query side as soon as events are persisted, but for
 ...
 * </ul>
 *
 * The Cassandra write journal is notifying the query side as soon as new `persistenceIds` are
 ...
 * in a [[akka.persistence.journal.Cassandra.Tagged]] with the given `tags`.
 *
 ...
 * </ul>
 *
 * The Cassandra write journal is notifying the query side as soon as tagged events are persisted, but for
 * efficiency reasons the query side retrieves the events in batches that sometimes can
 * be delayed up to the configured `refresh-interval` or given [[akka.persistence.query.RefreshInterval]]
 * hint.
 *
